...
export default function HihiMapStudioPage() {
  ...
  const [mapId, setMapId] = useState('chat1');
  const [mapList, setMapList] = useState<MapListEntry[]>([]);
  const [activeMapId, setActiveMapId] = useState('chat1');
  ...
  const refreshMapList = useCallback(async () => {
    if (!localOnly) return;
    try {
      const res = await fetch(`${STUDIO_URL}/api/hihi-maps`, {
        headers: {'X-Studio-Secret': secret},
      });
      if (!res.ok) return;
      const data = await res.json();
      if (Array.isArray(data.maps)) setMapList(data.maps as MapListEntry[]);
      if (typeof data.activeMapId === 'string') setActiveMapId(data.activeMapId);
    } catch {
  ...
  const loadMap = useCallback(
    async (id: string) => {
      ...
      try {
        if (online) {
          const res = await fetch(`${STUDIO_URL}/api/hihi-map?id=${encodeURIComponent(id)}`, {
            headers: {'X-Studio-Secret': secret},
          });
          if (res.ok) {
  ...
  const saveToServer = async () => {
    if (!localOnly) {
      window.alert('로컬 스튜디오에서만 저장할 수 있어요.');
      return;
    }
    const online = await pingStudio();
    if (!online) {
      window.alert('스튜디오 서버가 꺼져 있어요. npm run studio 후 다시 저장하세요.');
      return;
    }
    try {
      const id = 'chat1';
      mapRef.current = {...mapRef.current, name: id};
      setMapId(id);
      const res = await fetch(`${STUDIO_URL}/api/hihi-map`, {
        method: 'POST',
        headers: studioHeaders({'X-Studio-Secret': secret}),
        body: JSON.stringify({...mapRef.current, id, name: id}),
      });
      ...
      await fetch(`${STUDIO_URL}/api/hihi-map/active`, {
        method: 'POST',
        headers: studioHeaders({'X-Studio-Secret': secret}),
        body: JSON.stringify({activeMapId: id}),
      }).catch(() => null);
      setActiveMapId(id);
      setDirty(false);
      void refreshMapList();
  ...
  const setActiveMap = async () => {
    if (!localOnly) return;
    try {
      const res = await fetch(`${STUDIO_URL}/api/hihi-map/active`, {
        method: 'POST',
        headers: studioHeaders({'X-Studio-Secret': secret}),
        body: JSON.stringify({activeMapId: mapId}),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? `active ${res.status}`);
  ...
  const createNewMap = () => {
    const raw = window.prompt('새 맵 ID (영문/숫자/-/_ 만 가능, 예: chat2)', '');
    if (!raw) return;
    const id = raw.trim();
  ...
  const deleteCurrentMap = async () => {
    if (!localOnly || mapId === 'chat1') return;
    if (!window.confirm(`"${mapId}" 맵을 삭제할까요? 되돌릴 수 없어요.`)) return;
    try {
      const res = await fetch(`${STUDIO_URL}/api/hihi-map?id=${encodeURIComponent(mapId)}`, {
        method: 'DELETE',
        headers: studioHeaders({'X-Studio-Secret': secret}),
      });
      const data = await res.json().catch(() => ({}));
        ...
        <aside className="map-studio__side">
            ...
            >
              ...
              {mapList.map((m) => (
                <option key={m.id} value={m.id}>
                  {mapDisplayName(m.id)}{m.id === activeMapId ? ' ★' : ''}
                </option>
              ))}
            </select>
          ...
          <div className="map-studio__files">
            ...
            <button
              type="button"
              className="map-studio__btn"
              disabled={!localOnly || activeMapId === mapId}
              onClick={() => void setActiveMap()}
            >
              {activeMapId === mapId ? '게임 사용 중' : '이 맵을 게임에 적용'}
            </button>
          </div>
