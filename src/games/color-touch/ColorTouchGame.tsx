...
export default function ColorTouchGame() {
  ...
  const [message, setMessage] = useState('이 색깔을 콕! 눌러봐');
  ...
  return (
    <div className={`color-touch${flash ? ` color-touch--${flash}` : ''}`}>
      ...
      <div className="color-touch__score" aria-label={`별 ${score}개`}>
        {'★'.repeat(Math.min(score, 8))}
        <span className="color-touch__score-num">{score}</span>
      </div>
