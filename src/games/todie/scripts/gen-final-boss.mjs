/**
 * Make a distinct finalBoss sprite from boss.png (violet / pale-white recolor).
 * Run: node src/games/todie/scripts/gen-final-boss.mjs
 */
...
const OUT = path.join(MOBS, 'finalBoss.png');
...
function recolorFinalBoss(pixels) {
  ...
  for (let i = 0; i < pixels.length; i += 4) {
    ...
    let nb = b;
    if (isEye) {
      // icy white-violet glare
      nr = Math.min(255, 235 + (r - 180) * 0.15);
      ng = Math.min(255, 210 + (g - 160) * 0.1);
      nb = Math.min(255, 255);
    } else if (isGold) {
      // pale silver-violet trim
      nr = Math.round(r * 0.25 + 160);
      ng = Math.round(g * 0.22 + 140);
      nb = Math.round(b * 0.45 + 190);
    } else if (isBrightFur || isOrange) {
      // deep violet / indigo body
      nr = Math.min(255, Math.round(r * 0.35 + 70));
      ng = Math.round(g * 0.18 + 20);
      nb = Math.min(255, Math.round(b * 0.55 + 140));
    } else if (r < 60 && g < 60 && b < 70) {
      // armor → near-black with purple edge
      nr = Math.min(255, r + 28);
      ng = Math.min(255, g + 8);
      nb = Math.min(255, b + 42);
    }
...
const recolored = recolorFinalBoss(pixels);
