...
export function decodePng(buf) {
  ...
  else throw new Error(`unsupported png ct=${colorType} bit=${bitDepth}`);
...
export function processPngBuffer(buf) {
  const {w, h, pixels} = decodePng(buf);
  const t = trimOpaque(pixels, w, h, 1);
  return nearestFit(t.px, t.w, t.h, OUT_SIZE, OUT_SIZE);
}

export function writeProcessedPng(outPath, buf) {
  const px = processPngBuffer(buf);
  fs.mkdirSync(path.dirname(outPath), {recursive: true});
  fs.writeFileSync(outPath, encodePng(px, OUT_SIZE, OUT_SIZE));
}
